...
const getButtonStyles = ({ variant = 'filled', intent = 'primary', disabled }: ButtonBaseProps) => {
  return cn(
    ...
    // Filled variant
    variant === 'filled' && [
      'min-h-touch px-button-x py-button-y  border border-transparent shadow-sm',
    ...
    // Outlined variant
    variant === 'outlined' && [
      'min-h-touch px-button-x py-button-y  border',
      intent === 'primary' &&
        !disabled &&
    ...
    variant === 'text' && [
      ...
      intent === 'secondary' &&
        !disabled &&
        'text-muted border-line hover:text-ink hover:border-ink',
...
const renderChildren = (children: React.ReactNode) => {
  return React.Children.map(children, (child) => {
    ...
    if (React.isValidElement(child)) {
      ...
      if (
        childProps?.className?.includes('btn-text') ||
        childProps?.className?.includes('btn-icon')
      ) {
        return child;
      }
