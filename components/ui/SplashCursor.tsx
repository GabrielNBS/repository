...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const config = {
      ...
      CURL: CURL!,
      SPLAT_RADIUS: SPLAT_RADIUS! * 0.28,
      SPLAT_FORCE: SPLAT_FORCE! * 0.3,
      SHADING,
    ...
    function checkSectionAndUpdateConfig(clientX: number, clientY: number) {
      const target = document.elementFromPoint(clientX, clientY) as HTMLElement | null;
      const isOverProjects =
        target && (target.closest('#projects') || target.closest('.mockup-container'));

      if (isOverProjects) {
        config.SPLAT_RADIUS = SPLAT_RADIUS!;
        config.SPLAT_FORCE = SPLAT_FORCE!;
      } else {
        config.SPLAT_RADIUS = SPLAT_RADIUS! * 0.28;
        config.SPLAT_FORCE = SPLAT_FORCE! * 0.3;
      }
    }
    ...
    window.addEventListener('mousedown', (e) => {
      ...
      checkSectionAndUpdateConfig(e.clientX, e.clientY);
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        updateFrame();
        checkSectionAndUpdateConfig(touches[i].clientX, touches[i].clientY);
